...
const getNetworkFromChainId = (chainId?: number | string): string => {
  ...
  switch (chainIdNum) {
    ...
    case 56: return 'bsc';
    case 97: return 'bsc';
    default: return getNetworkSlugFromChainId(chainIdNum) || 'unknown';
  }
};
...
function NftCard({ nft, isSelected, onClick }: NftCardProps) {
  ...
  const imageUrl = getProcessedImageUrl(nft.imageUrl || nft.metadata?.image || '');
  ...
  const tryAlternativeImage = () => {
    if (nft.metadata?.image && nft.imageUrl && nft.metadata.image !== nft.imageUrl) {
      return getProcessedImageUrl(nft.metadata.image);
    }
    
    if (nft.metadata?.attributes) {
      const imageAttr = nft.metadata.attributes.find((attr: any) => 
        attr.trait_type?.toLowerCase() === 'image');
    ...
    >
      ...
      <CardContent>
        ...
        <Stack direction="row" spacing={1} mt={1} flexWrap="wrap">
          {network && (
            <Chip 
              label={networkInfo?.name || network.charAt(0).toUpperCase() + network.slice(1)} 
              size="small"
              sx={{ 
...
export function DirectNftSelector({ open, onClose, onSelect, selectedNft }: DirectNftSelectorProps) {
  ...
  const fetchNfts = async () => {
    ...
    try {
      ...
      if (response.data && response.data.length) {
        ...
        if (assets.length > 0) {
          const processedAssets = assets.map((asset: ExtendedAsset) => {
            if (!asset.imageUrl && asset.metadata?.image && isValidImageUrl(asset.metadata.image)) {
              asset.imageUrl = asset.metadata.image;
            }
            
            if (asset.imageUrl && asset.imageUrl.startsWith('ipfs://')) {
              asset.imageUrl = asset.imageUrl.replace('ipfs://', 'https://ipfs.io/ipfs/');
            }
          ...
          const networkCounts = processedAssets.reduce((acc: Record<string, number>, asset: any) => {
            const network = asset.networkId || 
              (asset.chainId ? getNetworkFromChainId(asset.chainId) : 'unknown');
            if (network) {
  ...
  const filteredNfts = useMemo(() => {
    ...
    if (searchTerm) {
      ...
      result = result.filter(nft => {
        ...
        const nameMatch = (nft.metadata?.name || nft.name) ? 
          (nft.metadata?.name || nft.name || '').toLowerCase().includes(searchLower) : false;
    ...
    result.sort((a, b) => {
      switch (sortBy) {
        case "name_asc":
          return (a.metadata?.name || a.name || '').localeCompare(b.metadata?.name || b.name || '');
        case "name_desc":
          return (b.metadata?.name || b.name || '').localeCompare(a.metadata?.name || a.name || '');
        case "network":
          const networkA = a.networkId || getNetworkFromChainId(a.chainId);
          const networkB = b.networkId || getNetworkFromChainId(b.chainId);
          return (networkA ?? 'unknown').toString().localeCompare((networkB ?? 'unknown').toString());
        case "chainId":
      ...
      <DialogTitle>
        <Stack direction="row" justifyContent="space-between" alignItems="center">
          <Typography variant="h6" fontWeight="bold">
            <FormattedMessage id="select.nft.for.profile" defaultMessage="Select NFT for Profile" />
          </Typography>
          ...
          <Grid item xs={12}>
            <Stack spacing={2}>
              <Box sx={{ 
                ...
                <Stack direction="row" spacing={1} flexWrap="wrap" sx={{ mt: 2, mb: 1 }} justifyContent="center">
                  ...
                  {SUPPORTED_NETWORKS.map(network => (
                    <Chip
                      key={network.id}
                      label={`${network.name} (${networkStats[network.id] || 0})`}
                      onClick={() => toggleNetwork(network.id)}
                      color={selectedNetworks.includes(network.id) ? "primary" : "default"}
                      variant={selectedNetworks.includes(network.id) ? "filled" : "outlined"}
                      size="small"
                      sx={{
                        '& .MuiChip-label': { 
                          fontWeight: selectedNetworks.includes(network.id) ? 'bold' : 'normal'
                        },
                  ...
                  <Box sx={{ display: "flex", justifyContent: "center", p: 4 }}>
                    <Stack alignItems="center" spacing={2}>
                      ...
                      <Typography variant="body2" color="text.secondary">
                        <FormattedMessage id="loading.nfts" defaultMessage="Loading your NFTs..." />
                      </Typography>
                    </Stack>
                  </Box>
                ) : error ? (
                  ...
                  <>
                    ...
                    <Grid container spacing={2}>
                      {paginatedNfts.map((nft, index) => (
                        <Grid item xs={6} sm={4} md={3} lg={2} key={`nft-${index}`}>
                          <NftCard
                            nft={nft}
      ...
      <DialogActions sx={{ p: 2, borderTop: '1px solid', borderColor: 'divider' }}>
        {selectedNft && (
          <Box sx={{ flexGrow: 1, display: 'flex', alignItems: 'center' }}>
            ...
            <Chip 
              label={selectedNft.metadata?.name || selectedNft.name || selectedNft.collectionName || 'NFT'} 
              color="primary" 
              size="small"
            />
          </Box>
