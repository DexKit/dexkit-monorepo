...
export default function ContractStakeErc20Container({
  ...
  const handleSubmitRewards = async ({
    ...
    const amountParsed = parseUnits(amount || '0.0', rewardTokenBalance?.decimals);
  ...
  const setRewardRatioMutation = useSetRewardRatio({ contract: contract as any });
                ...
                <Grid item xs={12}>
                  <Box>
                    <Stack direction="row" spacing={2}>
                      <Button
                        disabled={isSubmitting || !isValid || (!values.withdraw && !values.amount)}
                        startIcon={
                          isSubmitting ? (
