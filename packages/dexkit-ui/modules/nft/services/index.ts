import {
  ...
  ERC721Abi,
  IERC7572,
} from "@dexkit/core/constants/abis";
...
export async function getContractUriMetadata({
  contractURI,
}: {
  contractURI?: string;
}) {
...
export async function getApiMultipleAssets({
  query,
}: {
  query: any;
  ...
  const response = await dexkitNFTapi.post<AssetAPI[]>(
