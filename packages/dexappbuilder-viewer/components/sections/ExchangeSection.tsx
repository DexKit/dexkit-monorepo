    ...
    const tradeWidgetStyle = {
      ...
      "& .MuiTypography-root": {
        color: tradeWidgetTextColor
          ? `${tradeWidgetTextColor} !important`
          : undefined,
      },
      "& .MuiButton-root": {
        ...(tradeWidgetButtonColor && {
          ...
          <Box sx={{ display: previewContext?.isMobile ? "block" : "none" }}>
            <Box
              sx={{
                ...pairInfoStyle,
                backgroundColor:
                  pairInfoStyle?.backgroundColor || "rgba(255,255,255,0.1)",
                border:
                  pairInfoStyle?.border || "1px solid rgba(255,255,255,0.2)",
    ...
    const renderTradeWidgetComponent = (skipGridWrapper = false) => {
      ...
      return (
        <Grid
          item
          xs={12}
          md={showTradingGraph ? 6 : 12}
          lg={showTradingGraph ? 5 : 12}
          key="tradeWidget"
        >
          {content}
        </Grid>
    ...
    const renderTradingGraphComponent = (skipGridWrapper = false) => {
      ...
      return (
        <Grid
          item
          xs={12}
          md={showTradeWidget ? 6 : 12}
          lg={showTradeWidget ? 7 : 12}
          key="tradingGraph"
        >
          {content}
        </Grid>
      ...
      const glassmorphismDialogStyles = {
        "& .MuiDialog-paper": {
          ...
          "@media (max-width: 600px)": {
            ...
            backgroundColor: "rgba(255, 255, 255, 0.15) !important",
            backdropFilter:
              "blur(30px) saturate(200%) brightness(1.1) !important",
        ...
        '& .MuiBox-root[style*="position: sticky"]': {
          backgroundColor: "rgba(255, 255, 255, 0.1) !important",
          backdropFilter:
            "blur(25px) saturate(200%) brightness(1.08) !important",
          WebkitBackdropFilter:
            "blur(25px) saturate(200%) brightness(1.08) !important",
          borderBottom: "1px solid rgba(255, 255, 255, 0.25) !important",
          boxShadow:
            "0 4px 12px rgba(0, 0, 0, 0.05), 0 1px 0 rgba(255, 255, 255, 0.2) inset !important",
        },
